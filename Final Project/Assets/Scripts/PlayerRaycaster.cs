...
public class PlayerRaycaster : MonoBehaviour {
	// Event dispatcher for when player comes in or out of reach of an object.
	// When the player can reach an object.
	public delegate void ObjectInReachAction(GameObject reachObj);
	public static ObjectInReachAction OnObjectInReach;
	// When the player can no longer reach the object.
	...
	// The game object that the player is looking at, according to the raycast.
	...
	void FixedUpdate () {
		...
		if (Physics.Raycast (transform.position, transform.forward, out hit, interactDistance)) {
			...
			if (newReachObj != reachObj) {
				...
				// The player is in reach of a new object.
		...
		} else if (reachObj != null) {
			// The player is out of reach from the current object.
			Debug.Log ("[PlayerRaycaster OnObjectOutReach] " + reachObj.name + " went out of reach from player" );
			if (OnObjectOutReach != null) {
				OnObjectOutReach (reachObj);
			} else {
